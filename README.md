...
**Jointly Estimated Landmarking for Dynamic Survival Prediction**

`JEL` predicts survival risk dynamically from noisy longitudinal biomarkers.
...
remotes::install_github("kevinjwcho/JEL")

# or from a local source tree
# install.packages("path/to/JEL", repos = NULL, type = "source")
```
...
td <- JEL_dat(d, s = 5, var_list = var_list, h = 4, y_vars = y_vars)

## 2) fit (Epanechnikov kernel, fixed bandwidth h = 4)
fit <- JEL(td, y_vars = y_vars, s = 5, h = 4,
...
fit_tv <- JEL(td, y_vars = y_vars, s = 5, h = 4, base_terms = "drug",
              ker = "epanechnikov", Bs = list(spl, list(NULL)))
cb <- confBands.JEL(fit_tv, K = 1)   # pointwise confidence bands, marker 1
```
...
| Function | Purpose |
|---|---|
| `JEL_dat()` | Build the landmarked longitudinal + survival dataset at time `s`. |
| `JEL()` | Fit the model (fixed or time-varying via `Bs`). |
| `predict()` | Conditional survival / risk at horizon `tau`. |
| `AUCdyn()`, `PEdyn()` | Time-dependent AUC and prediction error. |
| `confBands.JEL()` | Confidence bands for time-varying coefficient functions. |
| `select_h_longitudinal()` | Cross-validated bandwidth selection. |

## Reference

Cho, J. and Chen, K. (2024). *Dynamic Prediction of Survival Outcomes Using a
Jointly Estimated Landmarking Approach.*
