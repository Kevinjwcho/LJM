#' Longitudinal LSCV bandwidth selection for LLAJEL
#'
#' Selects the kernel bandwidth \code{h} for the Local Linear Approximation
#' (LLA) step of \code{LLAJEL} by leave-one-observation-out cross-validation on
...
#' \code{var.e} are taken from a single baseline \code{LLAJEL} fit at
...
#' @param W Deprecated and ignored.  The LOO-CV criterion is now global (uses all
#'   observations); this argument is kept only so existing callers that still
#'   pass \code{W = ...} do not error.
...
#'   (\code{LLAJEL}).  Defaults to \code{median(h_grid)}.
#' @param fit Optional pre-fitted \code{"JEL"} object (the output of
#'   \code{\link{LLAJEL}}) at \code{baseline_h}.  If supplied, the baseline EM is
...
#'   \code{\link{LLAJEL}}.  Required when \code{fit} is \code{NULL}.
#' @param base_terms,Bs,gh.nodes,max.iter,tol Passed through to
#'   \code{\link{LLAJEL}} when running the baseline EM.
...
select_h_longitudinal <- function(
  ...
  # observation, not a window |t - s0| <= W around the landmark. The `W`
