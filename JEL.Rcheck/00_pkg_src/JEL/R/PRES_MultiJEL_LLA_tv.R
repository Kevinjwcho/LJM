...
PRES_hessian_LLA_tv <- function(Omega, data.mat, V, b, bhat, Sigmai, S,
  ...
  # Parameter-SCALED step (#2). Variance components in vech(D) are tiny
  # (~1e-3) and D's smallest eigenvalue (~1.5e-4) is comparable to a fixed
  # absolute delta (1e-4): perturbing a near-boundary vech(D) element by
  # +/-delta pushes D across the PD boundary, the score's eigenvalue floor
  # (pmax(eig,1e-10)) fires discontinuously, and the FD second-derivative
  # explodes (~1e23). Scaling the step to each parameter's magnitude keeps
  # the perturbation well inside the PD region for small variance params,
  # while leaving O(1) params (eta, phi, large D) effectively unchanged.
  h_floor <- 1e-8                              # min absolute step (avoid 0)
