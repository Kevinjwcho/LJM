...
## - Survival prediction for LLAJEL fitted objects
...
pred_surv_compute_LLA <- function(fitLLAJEL,
  ...
  if (is.null(fitLLAJEL$prep$s0)) stop("fitLLAJEL$prep$s0 is missing (landmark time).")
  ...
  if (!is.matrix(bLLA_new)) stop("bLLA_new must be a matrix.")
  if (ncol(bLLA_new) %% 2 != 0) stop("bLLA_new must have 2 columns per outcome (b0,b1).")
  if (is.null(colnames(bLLA_new))) stop("bLLA_new must have colnames (e.g., Y.1_b0, Y.1_b1, ...).")
  if (is.null(rownames(bLLA_new))) stop("bLLA_new must have rownames = subject ids (as character).")
  ...
  # -----------------------------
  # Align bLLA_new rows to Cox_dat.te (by id)
  # Cox_dat.te can be start/stop rows; we need per-row bLLA
  # -----------------------------
  id_te <- as.character(Cox_dat.te$id)
  idx   <- match(id_te, rownames(bLLA_new))
  if (anyNA(idx)) {
    bad <- unique(id_te[is.na(idx)])
    stop(paste0("bLLA_new is missing these ids: ", paste(bad, collapse = ", ")))
  }
  b_row <- bLLA_new[idx, , drop = FALSE]  # n_surv x (2*nK)

  eta_vec <- as.numeric(eta)
  q <- ncol(bLLA_new)   # = 2*nK
  ...
  if (is_tv) {
    ...
    # eta_b_mat[t, i] = etaBBs_mat[t, ] %*% bLLA_new[i, ]
