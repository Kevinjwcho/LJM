#' Create landmarked longitudinal and survival datasets for JEL/LLAJEL
#'
...
#' @param land_time Numeric scalar landmark time \eqn{s}. Subjects are kept only if
#'   \code{EvTime > s}.
...
#' @param min_window_obs Optional integer. If non-\code{NULL}, applies a
...
JEL_dat <- function(data, land_time, negative = FALSE,
                    var_list = list(id = "id", time = "time", EvTime = "EvTime", event = "event"),
                    h = NULL, min_window_obs = NULL, y_vars = NULL) {
  ...
  # ----------------------------
  if (!is.null(min_window_obs)) {
    if (is.null(h) || is.null(y_vars))
      stop("JEL_dat: min_window_obs requires both `h` and `y_vars`.")
  ...
  LMM_dat <- LMM_dat %>%
    ...
    dplyr::left_join(
      Surv_dat %>% dplyr::select(.data[[id]], start, stop, event),
      by = id
    )
  return(list(
    LMM_dat  = LMM_dat,
