...
mvlme <- function(d, Y, X, Z, Yk, Xk, Zk, mi, inits.long, K,
  ...
  beta <- inits.long$beta.init; beta.mat <- matrix(beta, nrow = K, byrow = T)
  ...
  names(params) <- c(rep("D", length(vech(D))),
                     paste0("var.e_", 1:K),
                     paste0(rep(paste0("beta_", 1:K), each=2), 0:1))
  ...
  while(diff > tol.mvlme){
    ...
    bmat <- lapply(b, function(x) matrix(x, ncol = 2, byrow = T))
    ...
    beta <- beta.new; beta.mat <- matrix(c(beta), nrow = K, byrow = T)
    V <- lapply(mi, function(i) {
